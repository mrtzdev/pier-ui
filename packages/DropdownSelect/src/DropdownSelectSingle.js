...
const DropdownSelectSingle = (props) => {
  ...
  const selectItem = (selectId) => {
    const findItem = () => {
      let item = options.find((x) => x.value === selectId);
      return item;
    };
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            {placeholder && Object.keys(selected).length === 0 ? (
              <div className="dropdown-placeholder">{placeholder}</div>
            ) : (
              <div className="dropdown-selected">{selected.label} </div>
            )}
...
export default DropdownSelectSingle;

DropdownSelectSingle.propTypes = propTypes;
DropdownSelectSingle.defaultProps = defaultProps;
